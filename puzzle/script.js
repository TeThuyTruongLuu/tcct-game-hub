...
function armTimerTriggers() {
	const startOnce = () => startTimer();
	document.querySelectorAll(".puzzle-piece, .puzzle-slot, .pieces-container, .puzzle-board").forEach(el => {
		el.addEventListener("pointerdown", startOnce, { once: true, capture: true });
		el.addEventListener("dragstart", startOnce, { once: true, capture: true });
		el.addEventListener("click", startOnce, { once: true, capture: true });
		el.addEventListener("touchstart", startOnce, { once: true, capture: true });
	});
}
...
function createPuzzle() {
	...
	previewOverlay.style.backgroundSize = "cover";
